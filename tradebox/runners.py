...
def loop_runner(runner_function_name):
    while True:
        ...
        if not runner_info["active"]:
            ...
            logger.info(f"Saved updated_runner_info for {runner_function_name}: {updated_runner_info}")

            time.sleep(updated_runner_info["adjusted_interval"])
            logger.info(f"Runner not active. Concluded interval pause for {runner_function_name}.")
            continue

        # Mark run for failure (it might succeed later)
        updated_runner_info["current_update_successful"] = 0
        database.update_runner(updated_runner_info)
        logger.info(f"Marked for failure: {runner_function_name}, current_update_successful set to 0")
        ...
        if success:
            ...
            # Update runner entry
            database.update_runner(updated_runner_info)
            logger.info(f"Runner ({runner_function_name}) successfully updated.")
            ...
            logger.info(f"Final decision on adjusted_interval: {updated_runner_info['adjusted_interval']} seconds")
